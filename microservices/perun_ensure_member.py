...
class PerunEnsureMember(ResponseMicroService):
    ...
    def process(self, context, data):
        if data.data['perun']['user']:
            ...
            raise SATOSAError(
                ...
                'configured PerunIdentity filter before this filter?'
            )
    ...
    def __handle_user(self, user, vo, data, context):
        ...
        member_status = self.__adapters_manager.get_member_status_by_user_and_vo(user, vo)  # noqa e501
        # TODO implement has_registration_form()
        vo_has_registration_form = self.__adapters_manager.has_registration_form(vo.id, PerunConstants.VO) # noqa e501
