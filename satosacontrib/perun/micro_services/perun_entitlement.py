...
class PerunEntitlement(ResponseMicroService):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        self.OUTPUT_ATTR_NAME = 'eduPersonEntitlement'
        self.RELEASE_FORWARDED_ENTITLEMENT = 'releaseForwardedEntitlement'
        self.FORWARDED_EDU_PERSON_ENTITLEMENT = 'forwardedEduPersonEntitlement'
        self.ENTITLEMENT_PREFIX_ATTR = 'entitlementPrefix'
        self.ENTITLEMENT_AUTHORITY_ATTR = 'entitlementAuthority'
        self.GROUP_NAME_AARC_ATTR = 'groupNameAARC'

        self.__config = config

        self.__group_mapping = self.__config['groupMapping']
        self.__extended = False
        if self.__config['entitlementExtended'] == 'true':
        ...
        self.__release_forwarded_entitlement = True
        if self.__global_cfg[self.RELEASE_FORWARDED_ENTITLEMENT] == 'false':
            self.__release_forwarded_entitlement = False

        self.__forwarded_edu_person_entitlement = \
            self.__global_cfg[self.FORWARDED_EDU_PERSON_ENTITLEMENT]

        self.__group_name_aarc = False
        if self.__config[self.GROUP_NAME_AARC_ATTR] == 'true':
            self.__group_name_aarc = True
    ...
    def __get_edu_person_entitlement(self, data):
        ...
        for group in groups:
            ...
            if self.__config['groupNameAARC'] \
    ...
    def __get_forwarded_edu_person_entitlement(self, data):
        ...
        if 'user' not in data.data['perun'] or not data.data['perun']['user']:
            self.__logger.debug(
                'perun:Entitlement: Object Perun User is not '
